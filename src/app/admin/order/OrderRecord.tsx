...
import deleteById from "@/pages/api/admin/order/delete";
...
const OrderRecord: React.FC<OrderProps> = ({ data, index, afterDelete}) => {
    ...
    const handleDeleteButtonClick = async (id: string) => {
        ...
        if (isConfirmed) {
            try {
                ...
                console.error("Error when deleting order ID:", id, error);
            }
        } else {
    ...
    return (
        <div className="flex items-center border-[1.2px] border-slate-200 bg-slate-50 rounded-sm p-2 transition hover:scale-105 hover:bg-slate-300 text-center text-sm">
            ...
            <div className="flex-1 ml-4 hidden">{data.userId}</div>
            ...
            <div className="flex-1 ml-4">{formatPrice(data.shippingFee)}</div>
