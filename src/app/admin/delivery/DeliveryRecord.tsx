...
import deleteById from "@/pages/api/admin/delivery/delete";
...
const DeliveryRecord: React.FC<DeliveryProps> = ({ data, index, afterDelete}) => {
    ...
    const handleDeleteButtonClick = async (id: string) => {
        ...
        if (isConfirmed) {
            try {
                await deleteById(id);
                console.log("Deleted delivery ID:", id);
                afterDelete();
            } catch (error) {
                console.error("Error when try to delete delivery ID:", id, error);
            }
        } else {
            console.log("Cancel deleting delivery ID:", id);
        }
    };
